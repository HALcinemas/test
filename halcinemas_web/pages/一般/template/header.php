<!-- header template -->
<div class="header">
	<img src="../img/cinemaLogo.png" alt="hal cinema logo" class="header-logo_img">
	<div class="header__primary">
		<a href="#" class="header__primary-menu">ログイン</a>
		<a href="#" class="header__primary-menu">新規登録</a>
	</div>
	<div class="header__nav">
		<ul class="header__nav--ul">
			<li class="header__nav--li"><a href="#" class="li-a">HOME</a></li>
			<li class="header__nav--li"><a href="#" class="li-a">THEATER</a></li>
			<li class="header__nav--li"><a href="#" class="li-a">MOVIE</a></li>
			<li class="header__nav--li"><a href="#" class="li-a">SERVICE</a></li>
			<li class="header__nav--li"><a href="#" class="li-a">CONTACT</a></li>
		</ul>
	</div>
</div>